        ...
        try {
            ...
            withCredentials([
                ...
                stage('Execute Hub Tests') {
                    try {
                        sh "mvn clean verify -Dskip.unit.tests -Dbrowser='name=Chrome&platform=Windows,name=Chrome&platform=Mac,name=Firefox&platform=Windows,name=Firefox&platform=Mac,InternetExplorer,Edge,Safari' -Dfailsafe.threads=30 -Dfailsafe.groups.exclude='service,local' -DappURL=http://34.233.135.10/ -Dhub=https://${sauceusername}:${saucekey}@ondemand.saucelabs.com"
