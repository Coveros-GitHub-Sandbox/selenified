...
public class Element {
	...
	public int click() {
		...
		try {
			...
			log.error(e);
			file.recordAction(action, expected, cantClick + prettyOutput() + ". " + e.getMessage(),
					Result.FAILURE);
			file.addError();
			return 1;
		}
	...
	public int submit() {
		...
		try {
			...
			log.error(e);
			file.recordAction(action, expected, cantSubmit + prettyOutput() + ". " + e.getMessage(),
					Result.FAILURE);
			file.addError();
			return 1;
		}
	...
	public int hover() {
		...
		try {
			...
			log.error(e);
			file.recordAction(action, expected, cantHover + prettyOutput() + ". " + e.getMessage(),
					Result.FAILURE);
			file.addError();
			return 1;
		}
	...
	public int blur() {
		...
		try {
			...
			log.error(e);
			file.recordAction(action, expected, cantFocus + prettyOutput() + ". " + e.getMessage(),
					Result.FAILURE);
			file.addError();
			return 1;
		}
		file.recordAction(action, expected, "Focused, then unfocused (blurred) on " + prettyOutput(),
				Result.SUCCESS);
		return 0;
	}
	...
	public int type(String text) {
		...
		String expected = prettyOutput() + " is present, displayed, and enabled to have text " + text
				+ " typed in";
		...
		try {
			...
			log.error(e);
			file.recordAction(action, expected, CANTTYPE + prettyOutput() + ". " + e.getMessage(),
					Result.FAILURE);
			file.addError();
			return 1;
		}
	 ...
	 *         steps
	 */
	public int type(Keys key) {
		String action = "Typing key '" + key + IN + prettyOutput();
		String expected = prettyOutput() + " is present, displayed, and enabled to have text " + key
				+ " entered";
		if (!isPresentEnabledInput(action, expected, CANTTYPE)) {
			return 1;
		}
	...
	public int clear() {
		...
		try {
			...
			log.error(e);
			file.recordAction(action, expected, cantClear + prettyOutput() + ". " + e.getMessage(),
					Result.FAILURE);
			file.addError();
			return 1;
		}
	...
	public int select(String value) {
		...
		try {
			...
			log.error(e);
			file.recordAction(action, expected, cantSelect + prettyOutput() + ". " + e.getMessage(),
					Result.FAILURE);
			file.addError();
			return 1;
		}
	 ...
	 *         steps
	 */
	private int cantMove(Exception e, String action, String expected) {
		log.error(e);
	 ...
	 *         steps
	 */
	private int isMoved(String action, String expected) {
		if (!is.displayed()) {
			file.recordAction(action, expected, prettyOutput() + " is not present on visible page",
					Result.FAILURE);
			file.addError();
	...
	public int move() {
		...
		try {
			...
			return cantMove(e, action, expected);
		}
		return isMoved(action, expected);
	}
