...
public class Wait {
	...
	public int forAlertPresent(double seconds) {
	    ...
	    if (!is.alertPresent()) {
	        file.recordAction(action, expected, WAITING + timetook + " seconds, an alert is not present", Result.FAILURE);
	        file.addError();
	        return 1;
	    }
	...
	public int forPromptPresent(double seconds) {
	    ...
	    double timetook = forPopup(seconds);
	    if (!is.promptPresent()) {
	        file.recordAction(action, expected, WAITING + timetook + " seconds, a prompt is not present", Result.FAILURE);
	        file.addError();
	        return 1;
	    }
